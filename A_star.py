...
class Graph:
    ...
    def find_way(self):
        ...
        while not queue.empty():
            ...
            if self.get_children(current):
                logging.debug(f'Вершина \'{current}\' имеет детей: ' + ','.join(self.get_children(current)))
            else:
...
while n:
    try:
        ...
        graph[begin][end] = [float(cost), abs(ord(end) - ord(finish))]
        n = input()
    except EOFError:
        break
